...
import { Copy, Gift, Link2, RefreshCw, UserPlus, WalletCards } from 'lucide-react'
  ...
  return (
    <div className="page invites-page">
      <header className="page-header invites-header">
        ...
        <button type="button" className="ghost-btn" onClick={() => void load()}><RefreshCw size={16} />{zh ? '刷新' : 'Refresh'}</button>
      </header>

      <div className="page-body invites-body">
        {error && <div className="auth-err">{error}</div>}
        <div className="invite-stats">
          <div><UserPlus size={20} /><span>{zh ? '累计邀请' : 'Referrals'}</span><strong>{data?.invitedCount ?? 0}</strong></div>
          <div><Gift size={20} /><span>{zh ? '当前返佣比例' : 'Commission rate'}</span><strong>{((data?.rateBps ?? 0) / 100).toFixed(2)}%</strong></div>
          <div><WalletCards size={20} /><span>{zh ? '累计佣金' : 'Total commission'}</span><strong>{money(data?.totalCommissionCents ?? 0)}</strong></div>
        </div>
        ...
        <section className="invite-panel">
          <div className="invite-panel-head"><div><h2>{zh ? '邀请码' : 'Codes'}</h2><p>{zh ? '可以创建多个邀请码并分别停用；0 次限制表示不限次数。' : 'Create multiple codes and disable them independently. Zero means unlimited.'}</p></div></div>
          <div className="invite-create-row">
            <input value={customCode} maxLength={24} placeholder={zh ? '自定义邀请码（可留空自动生成）' : 'Custom code (optional)'} onChange={(e) => setCustomCode(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, ''))} />
            <input type="number" min="0" value={maxUses} onChange={(e) => setMaxUses(e.target.value)} title={zh ? '最多使用次数，0 为不限' : 'Maximum uses; 0 is unlimited'} />
          ...
          <div className="invite-table-scroll">
            <table className="invite-table"><thead><tr><th>{zh ? '邀请码' : 'Code'}</th><th>{zh ? '使用次数' : 'Uses'}</th><th>{zh ? '状态' : 'Status'}</th><th>{zh ? '创建时间' : 'Created'}</th><th>{zh ? '操作' : 'Action'}</th></tr></thead>
              <tbody>{data?.codes.map((row) => <tr key={row.code}><td><code>{row.code}</code></td><td>{row.usedCount} / {row.maxUses || (zh ? '不限' : '∞')}</td><td><span className={`invite-status ${row.enabled ? 'on' : 'off'}`}>{row.enabled ? (zh ? '启用' : 'Active') : (zh ? '停用' : 'Disabled')}</span></td><td>{new Date(row.createdAt).toLocaleString()}</td><td><button type="button" className="ghost-btn small" onClick={async () => { await api.billing('setInviteEnabled', row.code, !row.enabled); await load() }}>{row.enabled ? (zh ? '停用' : 'Disable') : (zh ? '启用' : 'Enable')}</button></td></tr>)}</tbody>
            </table>
          </div>
        </section>

        <section className="invite-panel">
          <div className="invite-panel-head"><div><h2>{zh ? '邀请用户' : 'Referred users'}</h2><p>{zh ? '邮箱已做隐私遮罩，关系一旦建立不可修改。' : 'Emails are masked. Referral ownership cannot be changed.'}</p></div></div>
          <div className="invite-table-scroll"><table className="invite-table"><thead><tr><th>{zh ? '用户' : 'User'}</th><th>{zh ? '邀请码' : 'Code'}</th><th>{zh ? '注册时间' : 'Registered'}</th><th>{zh ? '贡献佣金' : 'Commission'}</th></tr></thead><tbody>
        ...
        <section className="invite-panel">
          <div className="invite-panel-head"><div><h2>{zh ? '返佣流水' : 'Commission ledger'}</h2><p>{zh ? '充值返佣和消费返佣分别记录，比例按发生时锁定。' : 'Top-up and spend commissions are recorded separately at the active rate.'}</p></div></div>
          <div className="invite-table-scroll"><table className="invite-table"><thead><tr><th>{zh ? '类型' : 'Type'}</th><th>{zh ? '来源用户' : 'User'}</th><th>{zh ? '计佣金额' : 'Base'}</th><th>{zh ? '比例' : 'Rate'}</th><th>{zh ? '佣金' : 'Commission'}</th><th>{zh ? '时间' : 'Time'}</th></tr></thead><tbody>

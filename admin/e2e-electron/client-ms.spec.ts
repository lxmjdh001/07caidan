...
test('Client Bahasa Melayu: halaman Pasukan dan navigasi dalam Bahasa Melayu', async () => {
  ...
  const app = await electron.launch({ executablePath: ELECTRON_PATH, args: [MAIN, `--user-data-dir=${join(tmpdir(), 'omni-ms-ignored')}`] })
  ...
  await win.locator('.rail-nav', { hasText: 'Urus Pasukan' }).click()

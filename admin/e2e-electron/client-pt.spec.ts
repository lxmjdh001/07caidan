...
test('Cliente em português: página Equipe e navegação em português', async () => {
  ...
  const app = await electron.launch({ executablePath: ELECTRON_PATH, args: [MAIN, `--user-data-dir=${join(tmpdir(), 'omni-pt-ignored')}`], env: { ...process.env, OMNI_USER_DATA: USER_DATA } })

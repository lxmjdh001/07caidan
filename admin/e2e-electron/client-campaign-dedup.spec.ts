...
test('客户端引流工单：配置判重库+时间规则并核对持久化', async () => {
  ...
  // 唯一库名/工单名：重粉库与工单都按 tenant 共享，固定名跨轮累积会触发严格模式多元素命中
  const libName = `判重库E2E${Date.now().toString(36).slice(-5)}`
  const campName = `判重配置工单E2E${Date.now().toString(36).slice(-5)}`
  ...
  await libCard.locator('input[type="text"]').first().fill(libName)
  await libCard.locator('textarea').first().fill('+15551110001\n+15551110002\n+15551110003')
  await libCard.getByRole('button', { name: '导入建库' }).click()
  await expect(win.getByText(libName).first()).toBeVisible({ timeout: 10_000 })
  ...
  await win.locator('input[placeholder="如：八月东南亚推广"]').fill(campName)
  ...
  await expect(
    editDedup.locator('.check-item', { hasText: libName }).locator('input[type="checkbox"]')

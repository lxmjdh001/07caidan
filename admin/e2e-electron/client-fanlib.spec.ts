...
test('客户端重粉库：粘贴名单导入建库并核对条数', async () => {
  ...
  const app = await electron.launch({ executablePath: ELECTRON_PATH, args: [MAIN, `--user-data-dir=${join(tmpdir(), 'omni-lib-ignored')}`], env: { ...process.env, OMNI_USER_DATA: USER_DATA } })

...
export function RegisteredUsersView({ client }: Props): React.JSX.Element {
  ...
  return <div className="users registered-users">
    ...
    <div className="table-scroll"><table className="users-table"><thead><tr><th>邮箱</th><th>类型</th><th>套餐</th><th>余额</th><th>到期时间</th><th>状态</th><th>注册时间</th><th>操作</th></tr></thead><tbody>
      {filtered.map((u) => <tr key={u.id} className={u.enabled ? '' : 'off'}><td><strong>{u.email}</strong>{u.ownerEmail && <small className="table-sub">子账号 · {u.ownerEmail}</small>}</td><td>{u.ownerId ? '客服' : '老板'}</td><td>{u.subscription?.planName || '未订阅'}</td><td>{money(u.balanceCents)}</td><td>{u.subscription ? dateTime(u.subscription.expiresAt) : '—'}</td><td><button className={`pill ${u.enabled ? 'on' : 'offp'}`} onClick={() => void toggle(u)}>{u.enabled ? '正常' : '已停用'}</button></td><td>{dateTime(u.createdAt)}</td><td className="row-actions"><button className="link-button" onClick={() => setEditing(u)}>编辑</button><button className="link-danger" onClick={() => void remove(u)}>删除</button></td></tr>)}
      {filtered.length === 0 && <tr><td colSpan={8} className="empty-cell">暂无注册用户</td></tr>}
    </tbody></table></div>

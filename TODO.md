...
- [~] 补齐目标市场：日✅ / 韩✅ / 繁中✅ / 越✅ / 泰 / 马来✅ / 印尼✅ / 西✅ / 葡(巴)✅ / 阿拉伯
      进度：**繁中 + 日 + 韩 + 越 + 印尼 + 马来 + 西 + 葡(巴) 均已 100%**（连同简中/英，共 10 语全量可用）。
      各 +200 余键（含 campaign/billing/team/settings/support/device 全部功能 + 意向标签），
      均 e2e 截图确认无英文回落。覆盖大中华/日韩/英语/东南亚/拉美(西+葡)全部主力打粉市场。
      仅剩泰(th)/阿拉伯(ar) 仍缺 ~185 键 —— 这两门（含 RTL）强烈建议母语校对后补齐
- [x] i18n 覆盖率 CI 守卫：scripts/i18n-coverage.mjs（npm run verify:i18n）—— 以 zh-CN 为规范键集，
      指定的 8 门完整语言(简中/英/繁中/日/韩/越/印尼/马来)若跌破 100% 即非零退出，防止新增键漏译回落英文；
      其余语言只报告覆盖率不阻断。4 条核心单测 + 语言切换器 e2e（设置里英→日实时生效，无需重启，截图核对）

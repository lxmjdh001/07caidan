...
- [ ] 上报内容：级别/消息/模块/应用版本/系统类型与版本/设备 ID/用户(或游客)
- [ ] 默认 warn 级别；后台可对指定用户/设备下发日志级别（客户端定期拉取生效）
- [ ] 未登录时以游客身份上报（仅设备 ID + 环境信息）
- [ ] 后台「客户端日志」页：按用户/设备/级别/版本筛选；日志级别控制入口
- [ ] 批量上报 + 失败本地缓冲重试；上报自身失败绝不影响功能

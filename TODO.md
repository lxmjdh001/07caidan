...
- [x] 实时/自动打标签（新消息触发意向重算）
      ...
      测试：11 条（关键词分级/落库/节流/上限/隔离）；3 个 e2e 截图（列表标签/意向筛选/面板自动展示）
- [ ] 媒体走对象存储（S3/MinIO）
...
- [~] 补齐目标市场：日 / 韩 / 繁中✅ / 越 / 泰 / 马来 / 印尼 / 西 / 葡(巴) / 阿拉伯
      进度：**繁体中文（台湾/香港）已 100% 补齐**（+207 键，用台湾用词：方案/餘額/點數/儲值/
      裝置/設定/登入/帳號 等；e2e 截图确认团队/导航等全繁体无英文回落）。
      其余 9 语仍缺 ~160-200/222 键（campaign/billing/team/settings/support/device 等回落英文）；
      日/韩/越/泰/马来/印尼/西/葡(巴)/阿拉伯建议逐语言补齐并经母语校对，键位已接好

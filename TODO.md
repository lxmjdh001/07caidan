...
## M16 — 托盘 / 版本与升级
- [ ] 托盘图标（mac 模板图标适配深浅菜单栏，Win 彩色）+ 菜单：
      打开主界面 / 未读数提示 / 检查更新 / 退出
- [ ] 关窗最小化到托盘（聊天软件惯例），托盘 tooltip 显示未读
- [x] 新消息系统通知（此前已完成：未聚焦弹通知/点击直达/可关预览/角标）
- [ ] electron-updater 自动更新：generic provider 指向自家后台 /updates，
      启动 + 定时检查，静默下载，退出时自动安装（"无感"），
      下载完成后界面温和提示"重启生效"
- [ ] 后台 /updates 静态分发路由（发布 = 把 electron-builder 产物扔进目录）
- [ ] 版本号展示：设置页关于块 + 托盘菜单；检查更新按钮与状态

## M17 — 运营公告与到期提醒
### 公告（管理员 → 定向人群）
- [ ] 公告表：标题/正文/受众/启停/有效期；受众类型：全部 / 指定套餐 /
      注册 N 天内的新用户 / N 天内到期的用户
- [ ] 受众在**拉取时**按用户实时求值（订阅/注册时间/到期时间），不做静态名单
- [ ] 已读表：客户端标记已读，未读才弹
- [ ] 管理后台「公告」页：新建/启停/受众选择；新权限 announcements:manage
- [ ] 客户端：启动与定时拉取未读公告，弹窗展示并标记已读
### 到期提醒
- [ ] 租户级设置：开关 / 提前天数（支持多档如 7,3,1）/ 是否发邮件 /
      邮件模板（主题+正文，支持 {{email}} {{planName}} {{expiresAt}}
      {{daysLeft}} {{appName}} 变量）
- [ ] 模板渲染纯函数 + 单测（缺变量保留原样不崩）
- [ ] 巡检并入 billing-cron：每档提前天数对每个到期周期只发一次（去重表）
- [ ] 站内提醒进个人通知，与公告同一拉取通道；邮件按开关发送
- [ ] 管理后台设置界面

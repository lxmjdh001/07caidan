...
### 套餐
- [ ] 套餐模型：名称、价格(USD)、**可登录账号数上限**、周期（月/季/半年/年，可自定义天数）
- [ ] 套餐订阅：生效时间、到期时间、状态（生效中/已过期/已取消）
- [ ] **自动续费**开关；到期自动从余额扣款，余额不足则降级并通知
- [ ] **升级/降级按时间折算，多退少补**：
      剩余价值 = 原套餐价 × 剩余天数/总天数；差额进/出余额
- [ ] 账号数超出新套餐上限时的处理策略（禁止新增 + 提示，不强制踢下线）
### 余额
- [ ] 用户余额（USD，整数分存储，避免浮点误差）
- [ ] 充值、消费、退款、折算差额的**流水账**（每一笔可追溯）
- [ ] 余额不足时的行为定义（停止续费 / 停止扣积分）
### 支付通道
- [ ] 易支付（彩虹易支付类协议：MD5 签名 + 异步回调）
- [ ] USDT（TRC20/ERC20：收款地址 + 链上确认，或第三方网关）
- [ ] PayPal（Orders v2 + Webhook 验签）
- [ ] 每个通道可配置**手续费率与固定费**
- [ ] 手续费可设为**由客户承担** → 下单/充值时把手续费加进应付金额
- [ ] 汇率表：USD → 各币种，管理员可维护；下单时锁定当次汇率并记录
- [ ] 回调幂等（同一订单重复回调只入账一次）

## M12 — AI 接口与模型积分
> 模型用途：客户端语音消息识别、收发文字翻译、后续 AI 自动回复。
- [ ] 管理后台配置 AI 供应商，至少支持 **OpenAI / Claude(Anthropic) / OpenRouter**
      三类协议（含自定义 baseUrl，兼容各种中转）
- [ ] 每个供应商下可配置多个模型，各自设定输入/输出 token 的**积分单价**
- [ ] 按 token 消耗扣**模型积分**；用量流水按用户/模型/用途分组可查
- [ ] 积分可开启**自动从余额扣除**；管理员设置余额↔积分兑换比例
- [ ] 用途区分：asr（语音识别）/ translate（翻译）/ autoreply（自动回复）
- [ ] 余额与积分都不足时的降级策略（停用付费模型，回落免费翻译引擎）
- [ ] 测试用假供应商（不真实调用模型，只按预设 token 数计费）
